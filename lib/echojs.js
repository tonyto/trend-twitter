...
_.extend(Echojs.prototype, {
	songSearch: function() {
	  var that = this
		...
		var songUrl = encodeURI(this.buildUrl('song/search'));
	...
	buildUrl: function(method) {
		return "/api/v4/{0}?api_key={1}&format=json&results=5&artist={2}&title={3}".format(method, this.apiKey, this.q['artist'], this.q['track'])
	}
});
